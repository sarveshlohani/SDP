...
object Main {
  ...
  def loadRddRawData(spark: SparkSession): (RDD[Medication], RDD[LabResult], RDD[Diagnostic]) = {
    ...
    val medication_df = CSVHelper.loadCSVAsTable(spark, "C:/Users/sarve/Desktop/OMSCS/BDGM/assignment3/hw3/hw3/code/data/medication_orders_INPUT.csv", "medication")
    val labResult_df = CSVHelper.loadCSVAsTable(spark, "C:/Users/sarve/Desktop/OMSCS/BDGM/assignment3/hw3/hw3/code/data/lab_results_INPUT.csv", "labResult")
    val encounter_dx_df = CSVHelper.loadCSVAsTable(spark, "C:/Users/sarve/Desktop/OMSCS/BDGM/assignment3/hw3/hw3/code/data/encounter_dx_INPUT.csv", "encounter_dx")
    val encounter_df = CSVHelper.loadCSVAsTable(spark, "C:/Users/sarve/Desktop/OMSCS/BDGM/assignment3/hw3/hw3/code/data/encounter_INPUT.csv", "encounter")

    labResult_df.na.drop()
    val medication_query = sqlContext.sql("SELECT Member_ID as patientID, to_date(Order_Date) as date, Drug_Name as medicine FROM medication")
    val labResult_query = sqlContext.sql("SELECT Member_ID as patientID, to_date(Date_Resulted) as date, Result_Name as testName, CAST(Numeric_Result as double) value FROM labResult where Numeric_Result!='' and Numeric_Result is not NULL")
    val diagnostic_query = sqlContext.sql("SELECT Member_ID as patientID, to_date(Encounter_DateTime) as date, code FROM encounter_dx, encounter where encounter_dx.Encounter_ID = encounter.Encounter_ID")

    val medication = medication_query.map(row => Medication(row(0).toString, row(1).asInstanceOf[java.sql.Date], row(2).toString)).rdd
    val labResult = labResult_query.map(row => LabResult(row(0).toString, row(1).asInstanceOf[java.sql.Date], row(2).toString, row(3).asInstanceOf[Double])).rdd
    val diagnostic = diagnostic_query.map(row => Diagnostic(row(0).toString, row(1).asInstanceOf[java.sql.Date], row(2).toString)).rdd
    //val medication = medication_query.as[Medication].rdd
    //val labResult = labResult_query.as[LabResult].rdd
    //val diagnostic = diagnostic_query.as[Diagnostic].rdd
    //medication_query.show(5)
    //labResult_query.show(5)
    //diagnostic_query.show(5)
    //medication.take(5)
    //labResult.take(5)
    //diagnostic.take(5)
    println("Medication count:" + medication.count())
    println("Lab result count:" + labResult.count())
    println("Diagnostic count:" + diagnostic.count())
