...
object T2dmPhenotype {
  ...
  def transform(medication: RDD[Medication], labResult: RDD[LabResult], diagnostic: RDD[Diagnostic]): RDD[(String, Int)] = {
    ...
    val T1DM_DX_rdd = sc.parallelize(T1DM_DX.toList)
    val T2DM_DX_rdd = sc.parallelize(T2DM_DX.toList)
    val T1DM_MED_rdd = sc.parallelize(T1DM_MED.toList)
    val T2DM_MED_rdd = sc.parallelize(T2DM_MED.toList)

    medication.take(5).foreach(println)
    diagnostic.take(5).foreach(println)
    labResult.take(5).foreach(println)

    val medication_patientId = medication.map(line => line.patientID).collect()

    val case_codes_dx = diagnostic.map(line => line.code).subtract(T1DM_DX_rdd).intersection(T2DM_DX_rdd).collect()

    diagnostic.map(line => line.code).foreach(println)
    diagnostic.map(line => line.code)
    case_codes_dx.take(5).foreach(println)
